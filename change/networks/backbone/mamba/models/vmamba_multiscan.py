...
from .sparse_scan import masked_cross_scan_v6, masked_cross_merge_v6
...
def cross_selective_scan_cpscan(
    ...
    #     idxs = scan_funcs[scan_type](H, W, 0, x.device)
    ...
    if mask == None:
        if scan_gen != None:
            y: torch.Tensor = scan_gen.merge(ys.view(B, K, -1, H * W))
        else:
...
def cross_selective_scan_v3(
    ...
    out_norm_shape="v0",
    out_proj: torch.nn.Module=nn.Identity(),
    pos_embed: torch.nn.Module=nn.Identity(),
    backw=1,
    channel_first=False,
    scan_type="", # None, snake, diagonal, zigzag, zorder, hilbert
    # ==============================
    ...
    def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
        return SelectiveScan.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, nrows, backnrows, ssoflex)
    softplus = nn.Softplus()
    xs, info = masked_cross_scan_v6(x, mask, pos_embed)
    L = xs.shape[-1]
    x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)
    if x_proj_bias is not None:
        x_dbl = x_dbl + x_proj_bias.view(B, K, -1, 1)
    dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
    dts = torch.einsum("b k r l, k d r -> b k d l", dts, dt_projs_weight)
    ...
    y = masked_cross_merge_v6(ys, info) # 还原形状,掩码区域补0
    L = H*W
...
class SS2D(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, scan_type=None, mask=None, out_proj=nn.Identity(), pos_embed=nn.Identity(), backw=1, scan_gen=None, **kwargs):
        ...
        if self.crossscan_version != 'v5':
            ...
            return cross_selective_scan_v3(
                x, mask, x_proj_weight, None, dt_projs_weight, dt_projs_bias,
                A_logs, Ds, delta_softplus=True,
                out_norm=out_norm,
                channel_first=self.channel_first,
    ...
    def forwardv2(self, x: torch.Tensor, scan_type=None, mask=None, out_proj=None, pos_embed=nn.Identity(), backw=1, scan_gen=None, **kwargs):
        ...
        y = self.forward_core(x, scan_type, mask, out_proj=out_proj, pos_embed=pos_embed, backw=backw, scan_gen=scan_gen)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
        return out
...
class VSSBlockMultiScan(nn.Module):
    ...
    def _forward(self, input: torch.Tensor, mask: torch.Tensor, out_proj: nn.Module=nn.Identity(), pos_embed: nn.Module=nn.Identity(), backw: torch.Tensor=1, scan_gen=None):
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(self.norm(self.op(input, self.scan_type, mask, out_proj=out_proj, pos_embed=pos_embed, backw=backw, scan_gen=scan_gen)))
            else:
                x = input + self.drop_path(self.op(self.norm(input), self.scan_type, mask, out_proj=out_proj, pos_embed=pos_embed, backw=backw, scan_gen=scan_gen))
        if self.mlp_branch:
            if self.post_norm:
    ...
    def forward(self, input: torch.Tensor, mask: torch.Tensor, out_proj=nn.Identity(), pos_embed=nn.Identity(),backw=1, scan_gen=None):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input)
        else:
            return self._forward(input, mask, out_proj, pos_embed, backw, scan_gen)
