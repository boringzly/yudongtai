...
def _load_checkpoint(model,
    ...
    patch_embed_weight = state_dict['patch_embed.conv_down.0.weight']
    for i in range(3, in_chans):
        patch_embed_weight = torch.cat(
            (patch_embed_weight, patch_embed_weight[:, (i % 3) : (i % 3 + 1), :, :]), 1
        )
    state_dict['patch_embed.conv_down.0.weight'] = patch_embed_weight
...
def mamba_vision_T(in_chans, pretrained=False, pretrained_path='', **kwargs):
    kwargs["in_chans"] = in_chans
    model_path = os.path.join(pretrained_path, "mamba/mambavision_tiny_1k.pth.tar")
    ...
    if pretrained:
        ...
        _load_checkpoint(model, model_path, in_chans)
    return model

def mamba_vision_T2(in_chans, pretrained=False, pretrained_path='', **kwargs):
    model_path = os.path.join(pretrained_path, "mamba/mambavision_tiny2_1k.pth.tar")
...
def mamba_vision_S(in_chans, pretrained=False, pretrained_path='', **kwargs):
    kwargs["in_chans"] = in_chans
    model_path = os.path.join(pretrained_path, "mamba/mambavision_small_1k.pth.tar")
...
def mamba_vision_B(in_chans, pretrained=False, pretrained_path='', **kwargs):
    kwargs["in_chans"] = in_chans
    model_path = os.path.join(pretrained_path, "mamba/mambavision_base_21k.pth.tar")
...
model_dict = {
    "mamba_vision_tiny": mamba_vision_T,
    "mamba_vision_tiny2": mamba_vision_T2,
    "mamba_vision_small": mamba_vision_S,
    "mamba_vision_base": mamba_vision_B,
}
channels_dict = {
    "mamba_vision_tiny": [640, 320, 160, 80],
    "mamba_vision_tiny2": [640, 320, 160, 80],
    "mamba_vision_small": [768, 384, 192, 96],
    "mamba_vision_base": [1024, 512, 256, 128],
}

def biuld_mamba_vision(model_name, in_chans, pretrained, pretrained_path):
    model = model_dict[model_name](in_chans, pretrained, pretrained_path)
    return model, channels_dict[model_name]

if __name__ == "__main__":
    mamba = mamba_vision_S(3, True, pretrained_path='/mnt/100/data/change_data/cp/cp_backup/32/pretrained_models/pretrained/', **default_cfgs['mamba_vision_S']).cuda(3)
    aa = torch.ones((2, 3, 512,512)).cuda(3)
    yy = mamba(aa)
    for y in yy:
        print(y.shape)