...
def test_lib_big_memeff(pre_img_path='', post_img_path='', output_path='', logger=None, callback_url=None, job_id=None,
    ...
    try:
        ...
        if pre_data is None:
            ...
            create_empty_shapefile(output_path, logger)
            # 创建完成标记文件
            create_completion_marker(output_path, logger)
            return
        ...
        if overlap_info is None:
            # 没有空间交集，直接创建空的shapefile和标记文件
            logger.info("两幅图像无空间交集，跳过推理，直接生成空结果")
    ...
    kwargs = {
        ...
        'TEST_CKPT_2': './FBCD_test_Levir_CD_best_acc.pth',
...
def test_lib(local_rank, nrank, cfg, logger):
    if logger is not None:
    ...
    # ---------- 加载两个模型 ----------
    # network 1 (first model)
    net1 = GenerateNet(cfg)
    pretrained_dict1 = torch.load(cfg.TEST_CKPT, map_location='cpu')
    module_model_state_dict1 = dict()
    for item, value in pretrained_dict1['model_state_dict'].items():
        if item.startswith('module.'):
            item = item[7:]
        module_model_state_dict1[item] = value
    net1.load_state_dict(module_model_state_dict1, strict=True)
    net1.to(device)
    
    # network 2 (second model)
    net2 = GenerateNet(cfg)
    pretrained_dict2 = torch.load(cfg.TEST_CKPT_2, map_location='cpu')
    module_model_state_dict2 = dict()
    for item, value in pretrained_dict2['model_state_dict'].items():
        if item.startswith('module.'):
            item = item[7:]
        module_model_state_dict2[item] = value
    net2.load_state_dict(module_model_state_dict2, strict=True)
    net2.to(device)
    
    if nrank > 1:
        net1 = torch.nn.parallel.DistributedDataParallel(net1, device_ids=[local_rank], output_device=local_rank)
        net2 = torch.nn.parallel.DistributedDataParallel(net2, device_ids=[local_rank], output_device=local_rank)
    net1.eval()
    net2.eval()
    ...
    for n in range(len(pre_namelist)):
        ...
        data_loader_test = torch.utils.data.DataLoader(test_data, cfg.TEST_BATCHES, shuffle=False, num_workers=1,
        ...
        for idx, (pre_img, post_img, nodata_mask, indexs) in enumerate(data_loader_test):
            ...
            if cfg.WITH_TTA:
                ...
                output1 = test_with_TTA(net1, images, cfg.MODEL_NUM_CLASSES, device)
                output2 = test_with_TTA(net2, images, cfg.MODEL_NUM_CLASSES, device)
            else:
                output1 = test_normal(net1, img, cfg.MODEL_NUM_CLASSES, device)  # (b,h,w)
