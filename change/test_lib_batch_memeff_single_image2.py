...
def test_lib(local_rank, nrank, cfg):
    ...
    for n in range(len(pre_namelist)):
        ...
        # import ipdb;ipdb.set_trace()
        tmp_idx = 0
        if gt_str_range_pre != gt_str_range_post and (proj == proj_post and res_gapx + res_gapy == 0 and gt_rot_pre == gt_rot_post):
            ...
            print(f"md pre path: {os.path.join(cfg.TEST_OUT_PATH, 'tmp', str(local_rank) + '_' + str(tmp_idx) + '_pre.tif')}")
            pre_tmp_save_path = os.path.join(cfg.TEST_OUT_PATH, 'tmp', str(local_rank) + '_' + str(tmp_idx) + '_pre.tif') 
            pre_data = gdal.Warp(
                    os.path.join(cfg.TEST_OUT_PATH, 'tmp', str(local_rank) + '_' + str(tmp_idx) + '_pre.tif'),
                    pre_data,
            ...
            if not Path(pre_tmp_save_path).exists():
                print(f"****** pre image {pre_namelist[n]}")
            # import sys
            # sys.exit() 
            post_data = gdal.Warp(
                    os.path.join(cfg.TEST_OUT_PATH, 'tmp', str(local_rank) + '_' + str(tmp_idx) + '_post.tif'),
        ...
        if Path(out_shp_namelist[0]).exists():
...
def run_parallel(pre_list, post_list, out_path, workers=0, logger=None, callback_url=None, job_id=None):
    ...
    tasks = list(zip(pre_list, post_list, out_path, logger, callback_url, job_id))
