...
def run_one_image_change_detection(input_path1, input_path2, mask_path, output_path, callback_url, job_id):
    try:
        ...
        # 复制input_path2到目标路径
        tmp_dir = "/cresdashare/docker/tmp/"
        os.makedirs(tmp_dir, exist_ok=True)  # 确保目标目录存在

        # 获取原始文件名
        original_filename = os.path.basename(input_path2)
        # 构建目标路径
        new_input_path2 = os.path.join(tmp_dir, original_filename)

        logger.info(f'正在复制文件: {input_path2} -> {new_input_path2}')
        # 复制文件
        shutil.copy2(input_path2, new_input_path2)
        logger.info(f'文件复制完成: {new_input_path2}')
        ...
        main_classification(input_path1, input_path2, mask_path, output_path, logger=logger, callback_url=callback_url,
                            job_id=job_id)

        post_progress(callback_url, post_status['finish'], 100, None, job_id)
    except Exception as e:
        print(str(e))
        error_message = {"errorCode": 4005, "message": "任务运行中报错" + str(e)}
...
def run_class_prediction():
    ...
    try:
        thread_pool_executor.submit(run_one_image_change_detection, input_path1, input_path2, mask_path, output_path,
                                    callback_url, job_id)
    except Exception as e:
        print("error", e)
