...
def infer(kwargs):
    ...
    message_dict = {'progress': 0, 'runningStatus': 'running', 'runningInfo': '框架初始化'}
    ...
    message_dict = {'progress': 0, 'runningStatus': 'running', 'runningInfo': 'GPU配置初始化'}
    prg_sender.send(message_dict)
    ...
    message_dict = {'progress': 0, 'runningStatus': 'running', 'runningInfo': 'GPU配置初始化完毕'}
    prg_sender.send(message_dict)

    message_dict = {'progress': 0, 'runningStatus': 'running', 'runningInfo': '加载参数配置'}
    prg_sender.send(message_dict)
    ...
    message_dict = {'progress': 1, 'runningStatus': 'running', 'runningInfo': '参数配置加载完毕'}
    prg_sender.send(message_dict)

    message_dict = {'progress': 1, 'runningStatus': 'running', 'runningInfo': '加载模型权重'}
    prg_sender.send(message_dict)
    ...
    message_dict = {'progress': 2, 'runningStatus': 'running', 'runningInfo': '加载模型权重完毕'}
    prg_sender.send(message_dict)

    message_dict = {'progress': 2, 'runningStatus': 'running', 'runningInfo': '构建文件列表'}
    prg_sender.send(message_dict)
    ...
    message_dict = {'progress': 3, 'runningStatus': 'running', 'runningInfo': '构建文件列表完毕'}
    prg_sender.send(message_dict)

    message_dict = {'progress': 3, 'runningStatus': 'running', 'runningInfo': '推理任务启动'}
    prg_sender.send(message_dict)
    ...
    for idx in range(len(basename_list_list[local_rank])):
        ...
        test_dataloader = DataLoader(test_data, cfg.TEST_BATCHES, shuffle=False, num_workers=cfg.DATA_WORKERS,
        ...
        for idx_x, (img, img_mask, idx_i, idx_j, status) in enumerate(test_dataloader):
            ...
            if proj_name in ['WGS 84', 'WGS 84 / Pseudo-Mercator']:
            ...
            message_dict = {'progress': _prg_idx, 'runningStatus': 'running', 'runningInfo': f'推理任务-{idx + 1}/{len(basename_list_list[local_rank])}',
                            'inferProgress': _sub_prg_idx, 'inferFilename': filename, 'inferGeoInfo': infer_geo_info}
            prg_sender.send(message_dict)
