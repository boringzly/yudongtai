...
class PAN(nn.Module):
    def __init__(self, loss_func, backbone_name, pretrained, os, num_band=3, num_class=10):
        ...
        self.gau_block1 = GAU(channels_blocks[0], channels_blocks[1],upsample=do_upsample[0])#upsample high_fm before mul with low_fm
        self.gau_block2 = GAU(channels_blocks[1], channels_blocks[2],upsample=do_upsample[1])#upsample X2
        self.gau_block3 = GAU(channels_blocks[2], channels_blocks[3], upsample = do_upsample[2])#upsample X2

        self.relu = nn.ReLU(inplace=True)

        self.upsample4=nn.Sequential(
                        nn.Conv2d( channels_blocks[3],64,kernel_size=3, stride=1, padding=1),
        ...
        self.cls_conv = nn.Conv2d(64, num_class, 1, 1, padding=0)
    ...
    def forward(self, x):
        ...
        low_layers = self.backbone(x[0])   #[x2,x4,x8,x16]
        fm_high = self.fpa(low_layers[3])
        fm_high = self.gau_block1(fm_high, low_layers[2])
        fm_high = self.gau_block2(fm_high, low_layers[1])
        fm_high = self.gau_block3(fm_high, low_layers[0])
        # fm_high = self.gau[3](fm_high, layers[0])
        ##need more conv to make edge more smooth
        
        output = self.cls_conv(self.upsample4(fm_high))
        if self.num_class > 1:
