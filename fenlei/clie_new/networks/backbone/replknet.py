...
class RepLKNet(nn.Module):
    ...
    def load_pretrained(self, pretrained_path):
        ...
        conv1_weight = old_dict["stem.0.conv.weight"]
        ...
        old_dict["stem.0.conv.weight"] = conv1_weight
        model_dict.update(old_dict)
        self.load_state_dict(model_dict)


def create_RepLKNet31B(in_channels=3, use_checkpoint=False, small_kernel_merged=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024],
                    drop_path_rate=0.3, small_kernel=5, use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged)

def create_RepLKNet31L(in_channels=3, use_checkpoint=False, small_kernel_merged=False):
