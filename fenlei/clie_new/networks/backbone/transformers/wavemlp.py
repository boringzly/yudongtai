...
class WaveNet(nn.Module):
    ...
    def load_pretrained(self, pretrained_path):
        old_dict = torch.load(pretrained_path, map_location="cpu")
        conv1_weight = old_dict["patch_embed.proj.weight"]
        ...
        old_dict["patch_embed.proj.weight"] = conv1_weight
...
# @register_model
def WaveMLP_T_dw(in_chans, **kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    model = WaveNet(layers, in_chans=in_chans, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                     mlp_ratios=mlp_ratios,mode='depthwise', **kwargs)
    model.default_cfg = default_cfgs['wave_T']
    return model    
    
# @register_model
def WaveMLP_T(in_chans, **kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
